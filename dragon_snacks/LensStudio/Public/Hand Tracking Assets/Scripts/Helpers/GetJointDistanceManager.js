    ...
    if (gestureMade) {
        ...
        if (!detectedGestureTriggered) {
            if (detectedTimer < triggerTimerMax) {
                ...
                // -----------------------------------------------------------------------
                // [author: Martina Kessler]: adapted this part to fix sendTriggers
                // (before it always sent triggers even though the checkbox was unchecked)
...
// -----------------------------------------------------------------------
// [author: Martina Kessler]: added these functions because they are used for deactivating/activating
// the triggers when the dragon is stunned/alive again
script.api.startSendingTriggers = function(){
    script.sendTriggers = true;
}

script.api.stopSendingTriggers = function(){
    script.sendTriggers = false;
}
// -----------------------------------------------------------------------
